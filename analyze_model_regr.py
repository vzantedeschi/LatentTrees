...
if DATA_NAME in ['GLASS', 'COVTYPE', 'ALOI']:
    ...
    true_y = np.hstack((data.y_train, data.y_valid, data._test))
    ...
    for l in model.latent_tree.bst.leaves:
        ...
        if len(y_distrs[l]) > 1 and model.latent_tree.bst.is_ancestor(t, l):
            ...
            if Y.ndim == 1:
                ...
                try:
                    sns.kdeplot(y_distrs[l], color=pal[l-model.latent_tree.bst.nb_split], shade=True, shade_lowest=False)
                except:
                    sns.distplot(y_distrs[l], color=pal[l-model.latent_tree.bst.nb_split], kde=False)
            
            elif y_distrs[l].size > 4:
                sns.kdeplot(y_distrs[l][:, 0], y_distrs[l][:, 1], color=pal[l-model.latent_tree.bst.nb_split], shade=True, shade_lowest=False)
